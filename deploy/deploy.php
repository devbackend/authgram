...
while(time() < PHP_INT_MAX) {
	...
	foreach ($releases as $release) {
		...
		writeToLog($deployHandler, '/usr/bin/git pull >> ' . $releaseDir . '/git.log');
		...
		writeToLog($deployHandler, '/usr/bin/composer install >> ' . $releaseDir . '/composer.log');
		...
		writeToLog($deployHandler, '/usr/local/bin/yarn install >> ' . $releaseDir . '/yarn.log');
		...
		writeToLog($deployHandler, '/usr/bin/php ' . __DIR__ . '/../artisan migrate --force >> ' . $releaseDir . '/migrate.log');
		...
		writeToLog($deployHandler, '/usr/local/bin/gulp --production >> ' . $releaseDir . '/gulp.log');
